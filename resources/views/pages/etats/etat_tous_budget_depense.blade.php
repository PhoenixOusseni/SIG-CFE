                ...
                <div class="card-body">
                    <div class="sbp-preview-content">
                        @foreach ($budgets as $budget)
                            @php
                                $somme_element = 0;
                                foreach ($budget->FactureFournisseur as $facture) {
                            ...
                            <table class="table table-bordered border-dark">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($budget->FactureFournisseur as $item)
                                        @foreach ($item->ElementFacture as $element)
                                            <tr>
                                                <td></td>
                                                <td>{{ $item->objet }}</td>
                                                <td>{{ number_format($element->montant_total, 0, ',', ' ') }}
                                                </td>
                                                <td>{{ $item->date }}</td>
                                            </tr>
                                        @endforeach
